        ...
        <flux:card>
            ...
            <form action="{{ route('notifikasi.update', $notifikasi->id) }}" method="POST" class="space-y-6">
                ...
                <flux:select name="type" label="Tipe Notifikasi (Kategori)" required>
                    <option value="pangkat" {{ old('type', $notifikasi->type) == 'pangkat' ? 'selected' : '' }}>Kenaikan Pangkat</option>
                    <option value="gaji_berkala" {{ old('type', $notifikasi->type) == 'gaji_berkala' ? 'selected' : '' }}>Kenaikan Gaji Berkala</option>
                    <option value="pensiun" {{ old('type', $notifikasi->type) == 'pensiun' ? 'selected' : '' }}>Persiapan Pensiun</option>
                </flux:select>
                ...
                <div class="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                    <flux:heading size="md" class="mb-3">Lampiran Dokumen (Dari {{ $notifikasi->employee->name }})</flux:heading>
                    ...
                    @if($notifikasi->sk_file_path)
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="text-xs text-zinc-500 mt-1">Diserahkan pada: {{ $notifikasi->submitted_at ? $notifikasi->submitted_at->format('d M Y, H:i') : '-' }} WIB</p>
                            </div>
                            <flux:button href="{{ asset('storage/' . $notifikasi->sk_file_path) }}" target="_blank" variant="outline" size="sm" class="text-emerald-600 hover:text-emerald-700 w-full sm:w-auto">
                ...
                <flux:select name="is_read" label="Status Keterbacaan Oleh Pegawai" required>
                    <option value="0" {{ old('is_read', $notifikasi->is_read) == 0 ? 'selected' : '' }}>Belum Dibaca</option>
                    <option value="1" {{ old('is_read', $notifikasi->is_read) == 1 ? 'selected' : '' }}>Sudah Dibaca</option>
                </flux:select>

                <div class="flex items-center justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                    <flux:button href="{{ route('notifikasi.index') }}" variant="subtle" wire:navigate>Batal</flux:button>
                    <flux:button type="submit" variant="primary" class="bg-emerald-600 hover:bg-emerald-700">Simpan Perubahan</flux:button>
                </div>
            </form>
        </flux:card>
    </div>
